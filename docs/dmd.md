...
For neon plasma DMD, the flicker/fusion threshold is estimated around **25Hz**, based on the analysis of existing
hardwares that have PWM frequencies down to 30Hz, but avoids to get lower (See Phantom Haus). This means that the perceived 
...
The flicker/fusion limit for LED DMD is unknown, perhaps higher as display is usually less bright which impacts Ferry/Porter
...
This hardware is made of discrete circuits with available datasheets, allowing to directly read & perform digital simulation. There has been
2 different board revision:
- PCA020 for "Al's Garahe Band Goes On a World Tour" which was modified after production by adding a clock divider to the rasterizer (existing schematics are incomplete),
- PCA020A which integrate this clock divider as well as other slight improvments for later games.

This is a simple rasterizer which rasterize each row 4 times, either from the same data source (no shading) or from 4 contiguous RAM locations allowing 0 / 25 / 50 / 75 / 100 shades.
...
This allows the hardware to render 0 / 33 / 66 / 100 shades.


## Data East 128x32 & Sega/Stern Whitestar

These systems share nearly identical hardwares: board 520-5055-00 for Data East, 520-5055-01 to 520-5055-03 for Whitestar.
...
They also show that the rasterizer setup changed between initial revision of the board (Data East) and later ones (Sega/Stern). Since the wiring
did not changed and the software seems similar, this leads to guess that the U2 chip setup was modified.

U16 is the main address decoder. It is a PAL16L8, which is a logic combiner (no flip flop). These are the guessed logic:
...
U2 drives the rasterizer by toogling the 6845 clock signal divider. It is a PAL16R4, which is a logic combiner with additional flip flops.
It's behavior is still largely unknown.
...
on SAM system, each row is rasterized 4 times with different lengths: 2 / 4 / 1 / 5 times 41.6µs. These timings lead to 12x41.6µs per 
...
Precise signal recordings are available from [RGB DMD project](https://github.com/ecurtz/RGB_DMD/tree/master/recordings). They show that 
Spike 1 system uses a PWM pattern made of 4 frames with different lengths: 1 / 2 / 4 / 8 times 1.05ms. These timings lead to 15.75ms for 
