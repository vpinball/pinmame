...
{	SRC_STATE	*state ;
	...
	((SRC_PRIVATE*) state)->mode = SRC_MODE_CALLBACK ;
	((SRC_PRIVATE*) state)->callback_func = func ;
	((SRC_PRIVATE*) state)->user_callback_data = cb_data ;
...
src_float_to_short_array (const float * const __restrict in, short * const __restrict out, int len)
{
	...
	for (i = 0 ; i < len ; i++)
#ifdef RESAMPLER_SSE_OPT
		out[i] = (short)_mm_cvtss_si32(_mm_max_ss(_mm_min_ss(_mm_set_ss(in[i] * 32768.f), _mm_set_ss(32767.f)), _mm_set_ss(-32768.f)));
#else
	{	float scaled_value ;
