...
#define LO_SURROGATE_START	0xDC00
				...
				gd3char_t X = (gd3char_t)srcstr[idxSrc];
				gd3char_t W = ((srcstr[idxSrc] >> 16) & 0x1F) - 1;
...
void vgm_start(struct RunningMachine *machine)
{
	...
	swprintf(vgmNotes, sizeof(vgmNotes)/sizeof(vgmNotes[0]), L"Generated by %hs %hs",
...
static void vgm_header_clear(uint16_t vgm_id)
{
	...
	Header->lngVersion = 0x151;	 // "base" version
...
//		break;
	...
	case VGMC_SEGAPCM:
	case VGMC_T6W28:
		vgm_header_sizecheck(chip_file, 0x151, 0x40);
		break;
	case VGMC_RF5C68:
	case VGMC_YM2203:
	case VGMC_YM2608:
	...
	case VGMC_PWM:
	case VGMC_AY8910:
		vgm_header_sizecheck(chip_file, 0x151, 0x80);
		break;
	case VGMC_GBSOUND:
	case VGMC_NESAPU:
	case VGMC_MULTIPCM:
	...
	case VGMC_QSOUND:
//	case VGMC_OKIM6376:
		vgm_header_sizecheck(chip_file, 0x161, 0xC0);
		break;
	case VGMC_SCSP:
		vgm_header_sizecheck(chip_file, 0x171, 0xC0);
		break;
	case VGMC_WSWAN:
	case VGMC_VSU:
	case VGMC_SAA1099:
	...
	case VGMC_X1_010:
	case VGMC_C352:
		vgm_header_sizecheck(chip_file, 0x171, 0xE0);
		break;
	case VGMC_GA20:
...
static void vgm_write_delay(uint16_t vgm_id)
{
	...
	while(VI->EvtDelay)
	{
		...
		{
			fputc(0x70 + (delaywrite - 1), VI->hFile);
			VI->BytesWrt += 0x01;
		}
		else
		{
