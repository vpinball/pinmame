 ...
 *
 *	 at91dasm.c
 *	 Portable ATMEL 91 ARM Thumb MCU Family Emulator ( ARM7TDMI Core ) - Disassembler
 *
 *   Chips in the family:
 ...
 *
 *****************************************************************************/
 /* NOTE: Instructions: CDP, LDC, MCR, STC (all co-processor related) AND SWP are not fully supported */
...
void at91_disasm( char *pBuf, data32_t pc, data32_t opcode )
{
	const char *pBuf0;
	...
	else if( (opcode&0x0f000000)==0x0e000000 )		//bits 27-24 == 1110
	{
		...
		// Todo
		if(opcode&0x10)
			pBuf += sprintf( pBuf, "COP-RT" );	//Reg Transfer
		else
			pBuf += sprintf( pBuf, "COP-DO" );	//Data Op
	}
