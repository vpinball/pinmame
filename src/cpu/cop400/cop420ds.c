...
//				flags = DASMFLAG_STEP_OVER;
	...
	{
		sprintf(buffer,"LBI 0%x",((op & 0xF) + 1) & 0xF);
	}
	else if (op >= 0x18 && op <= 0x1F)
	{
		sprintf(buffer,"LBI 1%x",((op & 0xF) + 1) & 0xF);
	}
	else if (op >= 0x28 && op <= 0x2F)
	{
		sprintf(buffer,"LBI 2%x",((op & 0xF) + 1) & 0xF);
	}
	else if (op >= 0x38 && op <= 0x3F)
	{
		sprintf(buffer,"LBI 3%x",((op & 0xF) + 1) & 0xF);
	}
	else if (op >= 0x51 && op <= 0x5F)
	{
		sprintf(buffer,"AISC %x",op & 0xF);
	}
...
//		flags = DASMFLAG_STEP_OVER;
	...
	else
	{
		...
		case 0x33:
			...
			{
				sprintf(buffer,"LBI 2,%x",op2 & 0xF);
			}
			else if (op2 >= 0xB1 && op2 <= 0xB8)
			{
				sprintf(buffer,"LBI 3%x",op2 & 0xF);
			}
			else
			{
				switch (op2)
