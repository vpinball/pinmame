...
static struct
{
   ...
   int nDisplays = 0; // For displays, most informations are stored in coreGlobals.dmdStates
   struct
   {
      DisplaySrcId srcId;
...
static DisplayFrame GetDisplayFrame(const CtlResId id)
{
   ...
   return { coreGlobals.dmdStates[id.resId].lumFrameId, coreGlobals.dmdStates[id.resId].dmdDotLum };
}
...
static DisplayFrame GetDisplayIdFrame(const CtlResId id)
{
   ...
   // TODO frames are directly updated in core_dmd_video_update (at emulation frequency)
   // This should be updated on request for correct shading (at render frequency)
   return { coreGlobals.dmdStates[id.resId].rawFrameId, coreGlobals.dmdStates[id.resId].dmdDotRaw };
}
...
static void SetupMsgApi()
{
   ...
   for (layout = core_gameData->lcdLayout, parent_layout = NULL; layout->length || (parent_layout && parent_layout->length); layout++) {
      ...
      case CORE_VIDEO: // Video display for games like Baby PacMan, frames are stored as RGB8
         ...
         {
            coreGlobals.dmdStates[msgLocals.nDisplays].dmdDotLum = new uint8_t[layout->start * layout->length * 3];
            if ((strncasecmp(Machine->gamedrv->name, "babypac", 7) == 0) // Baby Pacman (babypac, babypacn, babypaca)
               || (strncasecmp(Machine->gamedrv->name, "granny", 6) == 0))
            {
               // apply fixed rotation for Bally video pinballs
         ...
         {
            coreGlobals.dmdStates[msgLocals.nDisplays].dmdDotRaw = new uint8_t[layout->start * layout->length];
            coreGlobals.dmdStates[msgLocals.nDisplays].dmdDotLum = new uint8_t[layout->start * layout->length];
            msgLocals.displays[msgLocals.nDisplays].srcId.width = layout->length;
            msgLocals.displays[msgLocals.nDisplays].srcId.height = layout->start;
         }
         ...
         coreGlobals.dmdStates[msgLocals.nDisplays].width = layout->length;
         coreGlobals.dmdStates[msgLocals.nDisplays].height = layout->start;
         if ((layout->type & CORE_SEGMASK) == CORE_VIDEO)
...
static void ReleaseMsgApi()
{
   ...
   if (msgLocals.nDisplays > 0)
   {
	  ...
	  {
        delete[] coreGlobals.dmdStates[i].dmdDotRaw;
        delete[] coreGlobals.dmdStates[i].dmdDotLum;
        coreGlobals.dmdStates[i].dmdDotRaw = nullptr;		  
        coreGlobals.dmdStates[i].dmdDotLum = nullptr;		  
	  }
   }
