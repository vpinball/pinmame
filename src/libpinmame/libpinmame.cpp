...
static struct
{
   ...
   float segLuminances[16 * 128];
   float segPrevLuminances[16 * 128];
   ...
   unsigned char emptyDMD[DMD_MAXX * DMD_MAXY];
   unsigned int onDisplaySrcChangedId, getDisplaySrcId, getRenderDisplayId, getIdentifyDisplayId;
} msgLocals = { 0 };
...
extern "C" void OnStateChange(const int state)
{
	if (_isRunning != state)
	{
		...
		if (state == 1 && msgLocals.msgApi != NULL)
			msgLocals.msgApi->RunOnMainThread(0, OnGameStart, nullptr);
		else if (state == 0 && msgLocals.msgApi != NULL)
			msgLocals.msgApi->RunOnMainThread(0, OnGameEnd, nullptr);
...
static void onGetInputSrc(const unsigned int eventId, void* userData, void* msgData)
{
   ...
   if ((msgLocals.inputDef.nInputs > 0) && (msg->count < msg->maxEntryCount))
   {
...
{
   if (index >= msgLocals.inputDef.nInputs)
      return 0;
...
static void onGetDevSrc(const unsigned int eventId, void* userData, void* msgData)
{
   ...
   if ((msgLocals.deviceDef.nDevices > 0) && (msg->count < msg->maxEntryCount))
   {
...
{
   if (index >= msgLocals.deviceDef.nDevices)
      return 0;
...
static void onGetSegSrc(const unsigned int eventId, void* userData, void* msgData)
{
   ...
   for (unsigned int index = 0; (index < msgLocals.nSegDisplays) && (msg->count < msg->maxEntryCount); index++, msg->count++)
...
static void onGetSeg(const unsigned int eventId, void* userData, void* msgData)
{
   if (msgData == NULL)
      return;
   GetSegMsg* const msg = (GetSegMsg*)msgData;
   if ((msg->frame != nullptr) || (msg->segId.endpointId != msgLocals.endpointId) || (msg->segId.resId != 0))
      return;
   ...
   for (int i = 0; i < msgLocals.nSortedSegLayout; i++)
   {
      ...
      if ((type == CORE_SEG98F) || (type == CORE_SEG87F)) {
         ...
         for (int j = 0; j < nSegs; j++)
            if (j != 7 && msgLocals.segLuminances[i * 16 + j] > msgLocals.segLuminances[i * 16 + 7])
               msgLocals.segLuminances[i * 16 + 7] = msgLocals.segLuminances[i * 16 + j];
      }
      
      if (memcmp(&msgLocals.segPrevLuminances[i * 16], &msgLocals.segLuminances[i * 16], nSegs * sizeof(float)) != 0)
      {
         memcpy(&msgLocals.segPrevLuminances[i * 16], &msgLocals.segLuminances[i * 16], nSegs * sizeof(float));
         msgLocals.segFrameId++;
      }
   }
   msg->frame = msgLocals.segLuminances;
   msg->frameId = msgLocals.segFrameId;
}
...
{
   GetDmdSrcMsg* msg = (GetDmdSrcMsg*)msgData;
   for (unsigned int index = 0; (index < msgLocals.nDisplays) && (msg->count < msg->maxEntryCount); index++, msg->count++)
      memcpy(&msg->entries[msg->count], &msgLocals.displays[index].srcId, sizeof(DmdSrcId));
}

static void onGetIdentifyDisplay(const unsigned int eventId, void* userData, void* msgData)
{
   GetRawDmdMsg* const msg = (GetRawDmdMsg*)msgData;
   if ((msg->frame != nullptr) || (msg->dmdId.endpointId != msgLocals.endpointId) || (msg->dmdId.resId >= msgLocals.nDisplays))
      return;
   msg->format = ((core_gameData->gen & (GEN_SAM | GEN_SPA | GEN_ALVG_DMD2)) || (strncasecmp(Machine->gamedrv->name, "smb", 3) == 0) || (strncasecmp(Machine->gamedrv->name, "cueball", 7) == 0)) ? CTLPI_GETDMD_FORMAT_BITPLANE4 : CTLPI_GETDMD_FORMAT_BITPLANE2;
   if (coreGlobals.dmdStates[msg->dmdId.resId].dmdDotRaw == nullptr)
   {
      // Requested before first frame is available
      msg->frame = msgLocals.emptyDMD;
      msg->frameId = msgLocals.displays[msg->dmdId.resId].frameId;
      msg->width = msgLocals.displays[msg->dmdId.resId].srcId.width;
      msg->height = msgLocals.displays[msg->dmdId.resId].srcId.height;
   }
   else
   {
      msg->frame = (unsigned char*)coreGlobals.dmdStates[msg->dmdId.resId].dmdDotRaw;
      msg->frameId = msgLocals.displays[msg->dmdId.resId].frameId++; // TODO we could do some shared frame filtering here
      msg->width = coreGlobals.dmdStates[msg->dmdId.resId].width;
      msg->height = coreGlobals.dmdStates[msg->dmdId.resId].height;
   }
}

static void onGetRenderDisplay(const unsigned int eventId, void* userData, void* msgData)
{
   GetDmdMsg* const msg = (GetDmdMsg*)msgData;
   if ((msg->frame != nullptr)
    || (msg->dmdId.id.endpointId != msgLocals.endpointId)
    || (msg->dmdId.id.resId >= msgLocals.nDisplays)
    || (((coreGlobals.dmdStates[msg->dmdId.id.resId].layout->type & CORE_SEGMASK) == CORE_VIDEO) && (msg->dmdId.format != CTLPI_GETDMD_FORMAT_SRGB888))
    || (((coreGlobals.dmdStates[msg->dmdId.id.resId].layout->type & CORE_SEGMASK) == CORE_DMD) && (msg->dmdId.format != CTLPI_GETDMD_FORMAT_LUM8)))
      return;
   if (coreGlobals.dmdStates[msg->dmdId.id.resId].dmdDotLum == nullptr)
   {
      // Requested before first frame is available
      msg->frame = msgLocals.emptyDMD;
      msg->frameId = msgLocals.displays[msg->dmdId.id.resId].frameId;
   }
   else if ((coreGlobals.dmdStates[msg->dmdId.id.resId].width == msg->dmdId.width)
         && (coreGlobals.dmdStates[msg->dmdId.id.resId].height == msg->dmdId.height))
   {
      // TODO frames are directly updated in core_dmd_video_update (at emulation frequency)
      // This should be updated on request for correct shading (at render frequency)
      msg->frame = (unsigned char*)coreGlobals.dmdStates[msg->dmdId.id.resId].dmdDotLum;
      msg->frameId = msgLocals.displays[msg->dmdId.id.resId].frameId++; // TODO we could do some shared frame filtering here
   }
}
...
static void SetupMsgApi()
{
   ...
   msgLocals.onGameStartId = msgLocals.msgApi->GetMsgID(PMPI_NAMESPACE, PMPI_EVT_ON_GAME_START);
   msgLocals.onGameEndId = msgLocals.msgApi->GetMsgID(PMPI_NAMESPACE, PMPI_EVT_ON_GAME_END);

   // -- Prepare data structures for displays
   msgLocals.nDisplays = 0;
   int nSegLayouts = 0;
   const core_tLCDLayout* segLayout[128] = { 0 };
   const core_dispLayout* layout;
   const core_dispLayout* parent_layout;
   for (layout = core_gameData->lcdLayout, parent_layout = NULL; layout->length || (parent_layout && parent_layout->length); layout++) {
      ...
      case CORE_VIDEO: // Video display for games like Baby PacMan, frames are stored as RGB8
         assert(coreGlobals.dmdStates[msgLocals.nDisplays].layout == NULL);
         coreGlobals.dmdStates[msgLocals.nDisplays].layout = layout;
         msgLocals.displays[msgLocals.nDisplays].layout = layout;
         msgLocals.displays[msgLocals.nDisplays].srcId.id.endpointId = msgLocals.endpointId;
         msgLocals.displays[msgLocals.nDisplays].srcId.id.resId = msgLocals.nDisplays;
         msgLocals.displays[msgLocals.nDisplays].srcId.width = layout->length; // FIXME rotation is applied on video update which may change the definition
         msgLocals.displays[msgLocals.nDisplays].srcId.height = layout->start;
         msgLocals.displays[msgLocals.nDisplays].srcId.format = ((layout->type & CORE_SEGMASK) == CORE_VIDEO) ? CTLPI_GETDMD_FORMAT_SRGB888 : CTLPI_GETDMD_FORMAT_LUM8;
         if ((layout->type & CORE_SEGMASK) == CORE_VIDEO)
            msgLocals.displays[msgLocals.nDisplays].srcId.hardware = CTLPI_GETDMD_HARDWARE_UNKNOWN; // Report a CRT display ?
         else if ((layout->length < 128) || (layout->start < 16))
            msgLocals.displays[msgLocals.nDisplays].srcId.hardware = CTLPI_GETDMD_HARDWARE_UNKNOWN; // Mini display, usually LEDs
            ...
            msgLocals.displays[msgLocals.nDisplays].srcId.hardware = CTLPI_GETDMD_HARDWARE_UNKNOWN;
         else if (core_gameData->gen == GEN_SPA)
            msgLocals.displays[msgLocals.nDisplays].srcId.hardware = CTLPI_GETDMD_HARDWARE_UNKNOWN;
         else
            msgLocals.displays[msgLocals.nDisplays].srcId.hardware = CTLPI_GETDMD_HARDWARE_NEON_PLASMA;
         msgLocals.nDisplays++;
         break;
   ...
   {
      msgLocals.onDisplaySrcChangedId = msgLocals.msgApi->GetMsgID(CTLPI_NAMESPACE, CTLPI_ONDMD_SRC_CHG_MSG);
      msgLocals.getDisplaySrcId = msgLocals.msgApi->GetMsgID(CTLPI_NAMESPACE, CTLPI_GETDMD_SRC_MSG);
      msgLocals.getRenderDisplayId = msgLocals.msgApi->GetMsgID(CTLPI_NAMESPACE, CTLPI_GETDMD_RENDER_MSG);
      msgLocals.getIdentifyDisplayId = msgLocals.msgApi->GetMsgID(CTLPI_NAMESPACE, CTLPI_GETDMD_IDENTIFY_MSG);
      msgLocals.msgApi->SubscribeMsg(msgLocals.endpointId, msgLocals.getDisplaySrcId, onGetDisplaySrc, NULL);
      msgLocals.msgApi->SubscribeMsg(msgLocals.endpointId, msgLocals.getRenderDisplayId, onGetRenderDisplay, NULL);
      msgLocals.msgApi->SubscribeMsg(msgLocals.endpointId, msgLocals.getIdentifyDisplayId, onGetIdentifyDisplay, NULL);
   ...
   for (int i = 0; i < nSegLayouts; i++)
   {
      ...
      for (int j = 0; j < segLayout[i]->length; j++)
      {
         ...
         case CORE_SEG7:   type = CTLPI_GETSEG_LAYOUT_7; break;
         case CORE_SEG7S:  type = CTLPI_GETSEG_LAYOUT_7; break;
         case CORE_SEG7SC: type = CTLPI_GETSEG_LAYOUT_7; break;
         case CORE_SEG87F:
            type = isThousands ? CTLPI_GETSEG_LAYOUT_7C : CTLPI_GETSEG_LAYOUT_7;
            if (!isThousands)
               msgLocals.sortedSegLayout[msgLocals.nSortedSegLayout].srcType = CORE_SEG7 | (segLayout[i]->type & ~CORE_SEGALL);
            break;
         case CORE_SEG87:
            type = isThousands ? CTLPI_GETSEG_LAYOUT_7C : CTLPI_GETSEG_LAYOUT_7; break;
            if (!isThousands)
               msgLocals.sortedSegLayout[msgLocals.nSortedSegLayout].srcType = CORE_SEG7 | (segLayout[i]->type & ~CORE_SEGALL);
            else
               msgLocals.sortedSegLayout[msgLocals.nSortedSegLayout].srcType = CORE_SEG8 | (segLayout[i]->type & ~CORE_SEGALL);
            break;
         case CORE_SEG8:   type = CTLPI_GETSEG_LAYOUT_7C; break;
         case CORE_SEG8D:  type = CTLPI_GETSEG_LAYOUT_7D; break;
         case CORE_SEG9:   type = CTLPI_GETSEG_LAYOUT_9; break;
         case CORE_SEG10:  type = CTLPI_GETSEG_LAYOUT_9C; break;
         case CORE_SEG98F:
            type = isThousands ? CTLPI_GETSEG_LAYOUT_9C : CTLPI_GETSEG_LAYOUT_9;
            if (!isThousands)
         ...
         case CORE_SEG98:
            ...
            break;
         case CORE_SEG16N: type = CTLPI_GETSEG_LAYOUT_14; break;
         case CORE_SEG16D: type = CTLPI_GETSEG_LAYOUT_14D; break;
         case CORE_SEG16:  type = CTLPI_GETSEG_LAYOUT_14DC; break;
         case CORE_SEG16R: type = CTLPI_GETSEG_LAYOUT_14DC; break;
         case CORE_SEG16S: type = CTLPI_GETSEG_LAYOUT_16; break;
         }
   ...
   for (int i = 0; i < msgLocals.nSortedSegLayout; i++)
   {
         ...
         // end of block could also be a change of element size (based on element type) but does not seems to be used
      {
         ...
         msgLocals.segDisplays[msgLocals.nSegDisplays].srcId.id.endpointId = msgLocals.endpointId;
         msgLocals.segDisplays[msgLocals.nSegDisplays].srcId.id.resId = 0;
         msgLocals.segDisplays[msgLocals.nSegDisplays].srcId.displayIndex = msgLocals.nSegDisplays;
         ...
         case GEN_S9:
         case GEN_DE:
            msgLocals.segDisplays[msgLocals.nSegDisplays].srcId.hardware = CTLPI_GETSEG_HARDWARE_NEON_PLASMA;
            break;
         ...
         case GEN_GTS80B:
         case GEN_GTS3:
            msgLocals.segDisplays[msgLocals.nSegDisplays].srcId.hardware = msgLocals.segDisplays[msgLocals.nSegDisplays].srcId.nElements == 4 ? CTLPI_GETSEG_HARDWARE_GTS1_4DIGIT
                                                                         : msgLocals.segDisplays[msgLocals.nSegDisplays].srcId.nElements == 6 ? CTLPI_GETSEG_HARDWARE_GTS1_6DIGIT
                                                                         : msgLocals.segDisplays[msgLocals.nSegDisplays].srcId.nElements == 7 ? CTLPI_GETSEG_HARDWARE_GTS80A_7DIGIT
                                                                         : msgLocals.segDisplays[msgLocals.nSegDisplays].srcId.nElements == 20 ? CTLPI_GETSEG_HARDWARE_GTS80B_20DIGIT
                                                                         : CTLPI_GETSEG_HARDWARE_UNKNOWN; // This one should not happen but need to be checked (some playfield LED displays maybe ?)
            break;
            
         default:
            msgLocals.segDisplays[msgLocals.nSegDisplays].srcId.hardware = CTLPI_GETSEG_HARDWARE_UNKNOWN;
            break;
         }
   ...
   for (int i = 0; i < msgLocals.nSegDisplays; i++)
      msgLocals.segDisplays[i].srcId.nDisplaysInGroup = msgLocals.nSegDisplays;
   if (msgLocals.nSegDisplays > 0)
   {
      msgLocals.onSegSrcChangedId = msgLocals.msgApi->GetMsgID(CTLPI_NAMESPACE, CTLPI_ONSEG_SRC_CHG_MSG);
      msgLocals.getSegSrcId = msgLocals.msgApi->GetMsgID(CTLPI_NAMESPACE, CTLPI_GETSEG_SRC_MSG);
      msgLocals.getSegId = msgLocals.msgApi->GetMsgID(CTLPI_NAMESPACE, CTLPI_GETSEG_MSG);
      msgLocals.msgApi->SubscribeMsg(msgLocals.endpointId, msgLocals.getSegSrcId, onGetSegSrc, NULL);
      msgLocals.msgApi->SubscribeMsg(msgLocals.endpointId, msgLocals.getSegId, onGetSeg, NULL);
      msgLocals.msgApi->BroadcastMsg(msgLocals.endpointId, msgLocals.onSegSrcChangedId, NULL);
   }
   ...
   msgLocals.deviceDef.nDevices = nSols + coreGlobals.nGI + nLamps;
   ...
   for (int i = 0; i < nLamps; i++)
   {
      ...
      msgLocals.deviceDef.deviceDefs[nSols + coreGlobals.nGI + i].name = fmtString("Lamp #%02x", l);
   ...
   {
      msgLocals.onDevSrcChangedId = msgLocals.msgApi->GetMsgID(CTLPI_NAMESPACE, CTLPI_ONDEV_SRC_CHG_MSG);
      msgLocals.getDevSrcId = msgLocals.msgApi->GetMsgID(CTLPI_NAMESPACE, CTLPI_GETDEV_SRC_MSG);
   ...
   {
      msgLocals.onInputSrcChangedId = msgLocals.msgApi->GetMsgID(CTLPI_NAMESPACE, CTLPI_ONINPUT_SRC_CHG_MSG);
      msgLocals.getInputSrcId = msgLocals.msgApi->GetMsgID(CTLPI_NAMESPACE, CTLPI_GETINPUT_SRC_MSG);
...
static void ReleaseMsgApi()
{
   ...
   if (msgLocals.nDisplays > 0)
   {
      ...
      msgLocals.msgApi->UnsubscribeMsg(msgLocals.getRenderDisplayId, onGetRenderDisplay);
      msgLocals.msgApi->UnsubscribeMsg(msgLocals.getIdentifyDisplayId, onGetIdentifyDisplay);
      ...
      msgLocals.msgApi->ReleaseMsgID(msgLocals.getRenderDisplayId);
      msgLocals.msgApi->ReleaseMsgID(msgLocals.getIdentifyDisplayId);
   }
   ...
   if (msgLocals.nSegDisplays > 0)
   {
      ...
      msgLocals.msgApi->UnsubscribeMsg(msgLocals.getSegId, onGetSeg);
      ...
      msgLocals.msgApi->ReleaseMsgID(msgLocals.getSegId);
   }
...
static void OnGameStart(void*)
{
   if (msgLocals.msgApi == NULL)
      return;
   SetupMsgApi();
   PMPI_MSG_ON_GAME_START msg = { Machine->gamedrv->name };
