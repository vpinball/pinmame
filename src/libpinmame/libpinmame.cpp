...
static DisplayFrame GetDisplayFrame(const CtlResId id)
{
   ...
   if ((coreGlobals.dmdStates[id.resId].layout->type & CORE_SEGMASK) == CORE_VIDEO)
      return { coreGlobals.dmdStates[id.resId].rawFrameId, coreGlobals.dmdStates[id.resId].rawFrame };
   else
...
static void SetupMsgApi()
{
   ...
   for (layout = core_gameData->lcdLayout, parent_layout = NULL; layout->length || (parent_layout && parent_layout->length); layout++) {
      ...
      case CORE_VIDEO: // Video display for games like Baby PacMan, frames are stored as RGB8
         ...
         {
            coreGlobals.dmdStates[msgLocals.nDisplays].rawFrame = new uint8_t[layout->start * layout->length * 3];
         ...
         {
            coreGlobals.dmdStates[msgLocals.nDisplays].rawFrame = new uint8_t[layout->start * layout->length];
            coreGlobals.dmdStates[msgLocals.nDisplays].dmdDotLum = new float[layout->start * layout->length];
            msgLocals.displays[msgLocals.nDisplays].srcId.width = layout->length;
            msgLocals.displays[msgLocals.nDisplays].srcId.height = layout->start;
         }
...
static void ReleaseMsgApi()
{
   ...
   if (msgLocals.nDisplays > 0)
   {
      ...
      {
         delete[] coreGlobals.dmdStates[i].rawFrame;
         delete[] coreGlobals.dmdStates[i].dmdDotLum;
         coreGlobals.dmdStates[i].rawFrame = nullptr;
