...
get_lamp_descriptions(void) {
    ...
    while ((line = fgets(buffer, sizeof(buffer), fstream)) != NULL) {
        ...
        if (lamp_no < 36) {
            ...
            fprintf(stderr, "LISY1 Info: Lamp descriptions wrong info \n\r");
    }
}
...
get_switch_descriptions(void) {
    ...
    while ((line = fgets(buffer, sizeof(buffer), fstream)) != NULL) {
        ...
        if (switch_no < 80) {
            ...
            fprintf(stderr, "LISY1 Info: Switch descriptions wrong info \n\r");
    }
}
...
send_solenoid_infos(int sockfd) {
    ...
    //coil 1
    sprintf(buffer,
            "<form action=\'\' method=\'post\'><input type=\'submit\' name=\'C01\' %s value=\'%s\n%s\' /> </form>\n",
...
send_switch_infos(int sockfd) {
    ...
    do {
        ...
        if (ret
            < 80) //ret is switchnumber: NOTE: system has has 8*5==40 switches in maximum, counting 01..05;10...15; ...
        {
    ...
    for (i = 0; i <= 7; i++) //this is the return
    {
        ...
        for (j = 0; j <= 4; j++) //this is the strobe
        {
            ...
            if (!CHECK_BIT(swMatrixLISY1[j], i))
    ...
    sprintf(buffer, "<tr style=\"background-color:lawngreen;\" border=\"1\">\n");
    sendit(sockfd, buffer);
    if (!CHECK_BIT(swMatrixLISY1[6], 7))
        strcpy(colorcode, code_green);
    else
        strcpy(colorcode, code_red);
    sprintf(buffer, "%sSLAM<br></td>\n", colorcode);
    sendit(sockfd, buffer);
    if (!CHECK_BIT(swMatrixLISY1[6], 6))
        strcpy(colorcode, code_green);
    else
        strcpy(colorcode, code_red);
    sprintf(buffer, "%sOuthole<br></td>\n", colorcode);
    sendit(sockfd, buffer);
    if (!CHECK_BIT(swMatrixLISY1[6], 5))
        strcpy(colorcode, code_green);
    else
        strcpy(colorcode, code_red);
    sprintf(buffer, "%sRESET<br>onboard</td>\n", colorcode);
    sendit(sockfd, buffer);
    if (!CHECK_BIT(swMatrixLISY1[6], 4))
        strcpy(colorcode, code_green);
    else
        strcpy(colorcode, code_red);
...
main(int argc, char* argv[]) {
    ...
    if (ls80opt.bitv.JustBoom_sound) {
        ...
        sprintf(debugbuf, "/bin/echo \"Welcome to LISY 1 Control Version 0.%d\" | /usr/bin/festival --tts",
                LISY1control_SOFTWARE_SUB);
        system(debugbuf);
    }
