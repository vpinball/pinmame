...
do_sound_set(char* buffer) {
    ...
    if (sound_no >= 16) {
        ...
        if (lisy80_game.is80B)
            ...
            lisy80_coil_set(10, 1);
    } else {
        if (lisy80_game.is80B)
            lisy80_coil_set(5, 0);
        else
            lisy80_coil_set(10, 0);
    }
...
do_lamp_set(char* buffer) {
    ...
    //special lamp 78 means blinking lamps with pthread
        ...
        switch (lamp_no) {
            case 44:
                lisy80_coil_set(44 + 1, action);
                lamp[44] = action;
                lamp[48] = rev_action;
                break;
            case 45:
                lisy80_coil_set(45 + 1, action);
                lamp[45] = action;
                lamp[49] = rev_action;
                break;
            case 46:
                lisy80_coil_set(46 + 1, action);
                lamp[46] = action;
                lamp[50] = rev_action;
                break;
            case 47:
                lisy80_coil_set(47 + 1, action);
                lamp[47] = action;
                lamp[51] = rev_action;
                break;
            case 48:
                lamp[48] = action;
                lamp[44] = rev_action;
                lisy80_coil_set(44 + 1, rev_action);
                break;
            case 49:
                lamp[49] = action;
                lamp[45] = rev_action;
                lisy80_coil_set(45 + 1, rev_action);
                break;
            case 50:
                lamp[50] = action;
                lamp[46] = rev_action;
                lisy80_coil_set(46 + 1, rev_action);
                break;
            case 51:
                lamp[51] = action;
                lamp[47] = rev_action;
                lisy80_coil_set(47 + 1, rev_action);
                break;
            default:
...
get_lamp_descriptions(void) {
    ...
    while ((line = fgets(buffer, sizeof(buffer), fstream)) != NULL) {
        ...
        if (lamp_no < 52) {
            ...
            fprintf(stderr, "LISY80 Info: Lamp descriptions wrong info \n\r");
    }
}
...
get_switch_descriptions(void) {
    ...
    while ((line = fgets(buffer, sizeof(buffer), fstream)) != NULL) {
        ...
        if (switch_no < 80) {
            ...
            fprintf(stderr, "LISY80 Info: Switch descriptions wrong info \n\r");
    }
}
...
get_coil_descriptions(void) {
    ...
    while ((line = fgets(buffer, sizeof(buffer), fstream)) != NULL) {
        ...
        if (coil_no < 80) {
            ...
            fprintf(stderr, "LISY1 Info: coil descriptions wrong info \n\r");
    }
}

//send all the infos about the solenoids
void
send_solenoid_infos(int sockfd) {
    int i;
    ...
    for (i = 1; i <= 9; i++) {
        sprintf(
            buffer,
            "<form action=\'\' method=\'post\'><input type=\'submit\' name=\'C%02d\' %s value=\'%s\n%s\' /> </form>\n",
...
send_sound_infos(int sockfd) {
    ...
    for (j = 0; j <= 7; j++) {
        for (k = 0; k <= 3; k++) {
            ...
            sprintf(buffer,
                    "<form action=\'\' method=\'post\'><input type=\'submit\' name=\'S%02d\' %s value=\'Sound\n%02d\' "
                    "/> </form>\n",
                    i, colorcode, i);
            sendit(sockfd, buffer);
            //next sound
    ...
    for (i = 0; i <= 4; i++) {
        for (j = 0; j <= 9; j++) {
            ...
            if (lamp[lamp_no])
                ...
                sprintf(name, "L%02d_on", lamp_no);
            sprintf(buffer,
                    "<form action=\'\' method=\'post\'><input type=\'submit\' name=\'%s\' %s value=\'L%02d\n%s\n%s\' "
                    "/> </form>\n",
...
send_display_infos(int sockfd) {
    ...
    if (strcmp(lisy80_game.type_from_csv, "SYS80") == 0) {
        ...
        sprintf(
            buffer,
            "<p>Display Player 1: <input type=\"text\" name=\"D1A\" size=\"7\" maxlength=\"7\" value=\"%7s\" /></p>",
            display_D1);
        //sprintf(buffer,"<p>Display Player 1: <input type=\"text\" name=\"D1A\" size=\"7\" maxlength=\"7\" /></p>");
        sendit(sockfd, buffer);
        sprintf(
            buffer,
            "<p>Display Player 2: <input type=\"text\" name=\"D2A\" size=\"7\" maxlength=\"7\" value=\"%7s\" /></p>",
            display_D2);
        //sprintf(buffer,"<p>Display Player 2: <input type=\"text\" name=\"D2A\" size=\"7\" maxlength=\"7\" /></p>");
        sendit(sockfd, buffer);
        sprintf(
            buffer,
            "<p>Display Player 3: <input type=\"text\" name=\"D3A\" size=\"7\" maxlength=\"7\" value=\"%7s\" /></p>",
            display_D3);
        //sprintf(buffer,"<p>Display Player 3: <input type=\"text\" name=\"D3A\" size=\"7\" maxlength=\"7\" /></p>");
        sendit(sockfd, buffer);
        sprintf(
            buffer,
            "<p>Display Player 4: <input type=\"text\" name=\"D4A\" size=\"7\" maxlength=\"7\" value=\"%7s\" /></p>",
            display_D4);
        //sprintf(buffer,"<p>Display Player 4: <input type=\"text\" name=\"D4A\" size=\"7\" maxlength=\"7\" /></p>");
        sendit(sockfd, buffer);
        sprintf(buffer,
                "<p>Display Player 5: <input type=\"text\" name=\"D5\" size=\"6\" maxlength=\"6\" value=\"%7s\" /></p>",
                display_D5);
        //sprintf(buffer,"<p>Display Player 5: <input type=\"text\" name=\"D5\" size=\"6\" maxlength=\"7\" /></p>");
        sendit(sockfd, buffer);
        sprintf(buffer,
                "<p>Display Player 6: <input type=\"text\" name=\"D6\" size=\"6\" maxlength=\"6\" value=\"%7s\" /></p>",
                display_D6);
        ...
        sprintf(
            buffer,
            "<p>Display Reihe 1: <input type=\"text\" name=\"D1B\" size=\"20\" maxlength=\"20\" value=\"%20s\" /></p>",
            display_row1);
        //sprintf(buffer,"<p>Display Reihe 1: <input type=\"text\" name=\"D1B\" size=\"20\" maxlength=\"20\" /></p>");
        sendit(sockfd, buffer);
        sprintf(
            buffer,
            "<p>Display Reihe 2: <input type=\"text\" name=\"D2B\" size=\"20\" maxlength=\"20\" value=\"%20s\" /></p>",
            display_row2);
...
send_switch_infos(int sockfd) {
    ...
    for (i = 0; i <= 7; i++) {
        ...
        for (j = 0; j <= 7; j++) {
            ...
            if (lisy80_gtb_socket_switches[switch_no])
...
send_home_infos(int sockfd) {
    ...
    sendit(sockfd, buffer);
    sprintf(buffer, "This is LISY80control version %d.%d<br>\n", LISY80control_SOFTWARE_MAIN,
            LISY80control_SOFTWARE_SUB);
    sendit(sockfd, buffer);
    sprintf(buffer, "Selected game is %s, internal number %d<br><br>\n", lisy80_game.long_name, lisy80_game.gamenr);
    sendit(sockfd, buffer);
...
main(int argc, char* argv[]) {
    ...
    for (i = 0; i < 80; i++)
        lisy80_gtb_socket_switches[i] = 1;
    ...
    for (i = 0; i <= 47; i++)
        lamp[i] = 0;
    for (i = 48; i <= 51; i++)
        lamp[i] = 1; //reversed lamps 44,45,46,47
    ...
    if (ls80opt.bitv.JustBoom_sound) {
        ...
        sprintf(debugbuf, "/bin/echo \"Welcome to LISY 80 Control Version 0.%d\" | /usr/bin/festival --tts",
                LISY80control_SOFTWARE_SUB);
        system(debugbuf);
    }
    ...
    } else //we found an IP
    {
        ...
        if (lisy80_game.is80B) {
            ...
            display_show_str(1, buffer);
            strcpy(display_D1, buffer);
            fprintf(stderr, "%s\n", buffer);
            sprintf(buffer, "%-6s", strtok(NULL, "."));
            display_show_str(2, buffer);
            strcpy(display_D2, buffer);
            fprintf(stderr, "%s\n", buffer);
            sprintf(buffer, "%-6s", strtok(NULL, "."));
            display_show_str(3, buffer);
            strcpy(display_D3, buffer);
            fprintf(stderr, "%s\n", buffer);
            sprintf(buffer, "%-6s", strtok(NULL, "."));
            display_show_str(4, buffer);
            strcpy(display_D4, buffer);
            fprintf(stderr, "%s\n", buffer);
        } //80 or 80A
    }
    ...
    do {
        ...
        else if (strcmp(buffer, "solenoids") == 0) {
            send_solenoid_infos(newsockfd);
            close(newsockfd);
        }
