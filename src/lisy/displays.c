...
*/
void
display1_show_int(int display, int digit, int dat) {
    int i;
    char datchar;
    ...
    if (ls80opt.bitv.sevendigit) {
        ...
        if ((strcmp(&player[display - 1][1], "      ") == 0) & (player[display - 1][0] != ' ')) {
            ...
            if (ls80dbg.bitv.displays)
                lisy80_debug("7digit: add a space outmost left");
        ...
        if ((strcmp(&player[display - 1][1], "000000") == 0) & (player[display - 1][0] != '0')) {
            if (ls80dbg.bitv.displays)
                lisy80_debug("7digit: add a ZERO outmost left");
        ...
        if ((player[display - 1][6] == ' ') & (player[display - 1][5] != ' ')) {
            if (ls80dbg.bitv.displays)
                lisy80_debug("7digit: add a space secind digit");
        ...
        if ((player[display - 1][6] == '0') & (player[display - 1][5] != '0')) {
            if (ls80dbg.bitv.displays)
                lisy80_debug("7digit: add a ZERO secind digit");
...
*/
void
display_show_int(int display, int digit, int dat) {
    char datchar;
...
*/
void
display_show_char(int display, int digit, char data) {
...
*/
void
display35_show_int(int display, int digit, unsigned char dat) {
    ...
    if (lisy_hardware_revision == 200)
        wheels_show_int(display, digit, dat, 0);
}
...
*/
void
display_show_str(int display, char* data) {
...
*/
void
display35_show_str(int display, char* data) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (len <= 6)
            display35_show_int(display, i + 2, data[i] - 48);
        else
            display35_show_int(display, i + 1, data[i] - 48);
    }
}
...
void
display_cmd2pic_noansw(unsigned char command) {
...
int
display_cmd2pic(unsigned char command) {
...
void
display_cmd2pic_80b_noansw(unsigned char rows, unsigned char value) {
...
void
display_multibyte_to_pic_80b_noansw(unsigned char rows, char* buf, int no) {
    /* build control byte */
    mydata_display.bitv.IS_CMD = 1; //we are sending a command here
...
void
display_send_byte_torow(unsigned char data, int LD1_was_set, int LD2_was_set) {
...
void
display_send_row_torow_bp(char* buf, int LD1_was_set, int LD2_was_set) {
...
//send one row (20 chars) to display, to rigth row
void
display_send_row_torow(char* buf, int LD1_was_set, int LD2_was_set) {
...
void
display_sendtorow(unsigned char data, int LD1_was_set, int LD2_was_set, int debug) {
    ...
    if (last_data_was_ctrl == 1) {
        ...
        switch (data) {
            case 5:
                if (debug)
                    lisy80_debug("Set digit time to 16 cycles per grid\n\r");
                display_send_byte_torow(data, LD1_was_set, LD2_was_set);
                break;
            case 6:
                if (debug)
                    lisy80_debug("Set digit time to 32 cycles per grid\n\r");
                display_send_byte_torow(data, LD1_was_set, LD2_was_set);
                break;
            case 7:
                if (debug)
                    lisy80_debug("Set digit time to 64 cycles per grid\n\r");
                display_send_byte_torow(data, LD1_was_set, LD2_was_set);
                break;
            case 8:
                if (debug)
                    lisy80_debug("Enable Normal Display Mode (MSB in data words is cursor control only\n\r");
                display_send_byte_torow(data, LD1_was_set, LD2_was_set);
                break;
            case 9:
                if (debug)
                    lisy80_debug(
                        "Enable Blank Mode (data words with MSB = 1 will be blanked and cursor will be on\n\r");
                display_send_byte_torow(data, LD1_was_set, LD2_was_set);
                break;
            case 0x0a:
                if (debug)
                    lisy80_debug(
                        "Enable inverse Mode (data words with MSB = 1 will be inversed and cursor will be on\n\r");
                display_send_byte_torow(data, LD1_was_set, LD2_was_set);
                break;
            case 0x0e:
                if (debug)
                    lisy80_debug("start display refresh cycle\n\r");
                display_send_byte_torow(data, LD1_was_set, LD2_was_set);
                break;
            case 0x40 ... 0x7f:
                if (debug) {
                    sprintf(debugbuf, "load duty cycle:%d\n\r", data & 0x3f);
            ...
            case 0x80 ... 0x9f:
                if (debug) {
                    sprintf(debugbuf, "load digit counter:%d\n\r", data - 0x80);
                    lisy80_debug(debugbuf);
                }
                display_send_byte_torow(data, LD1_was_set, LD2_was_set);
                break;
            case 0xc0 ... 0xdf:
                if (debug)
                    fprintf(stderr, "load buffer pointer reg with:%d\n\r", data & 0x1f); //lower five bits
            ...
            default:
                if (debug) {
                    sprintf(debugbuf, "unknown command after ctrl:%d \n\r", data);
                    lisy80_debug(debugbuf);
                }
        } //switch
    } else {
        if (LD1_was_set) {
            ...
            if (row1_counter == 20) {
                if (memcmp(row1, oldrow1, 20 * sizeof(char)) != 0) {
                    ...
                    if (debug) {
                        sprintf(debugbuf, "[row 1 changed][%s]\n\r", row1);
                        lisy80_debug(debugbuf);
                        //int ii;
            ...
            //error?
            if (row1_counter > 20) {
                fprintf(stderr, "ERROR row1 counter overflow\n\r");
                row1_counter = 0;
            }
        }
        if (LD2_was_set) {
            ...
            //error?
            if (row2_counter > 20) {
                fprintf(stderr, "ERROR row2 counter overflow\n\r");
                row2_counter = 0;
            }
        }
    }
...
void
display_show_error_message(int errornumber, char* msg80, char* msg80B) {
...
void
display_show_pic_sw_version(int pic_no, int sw_version) {
...
void
display_show_boot_message_lisy35(char* s_lisy35_software_version) {
...
void
display_show_boot_message_lisy1(char* s_lisy1_software_version, char* rom_id, char* lisy1_gamename) {
    //int i;
    char buf[22];
...
void
display_show_boot_message(char* s_lisy80_software_version, int lisy80_gtb_no, char* lisy80_gamename) {
    int i;
    char buf[22];
    ...
    if (lisy80_game.is80B) {
        ...
        for (i = 0; i < 8; i++)
            lisy80_gamename[i] = toupper(lisy80_gamename[i]);
        //line 1 countdown
        for (i = 5; i >= 0; i--) {
...
void
display_show_shut_message(void) {
...
void
lisy35_display_set_soundE(unsigned char value) {
    if (value)
        display_cmd2pic_noansw(LS35_SOUNDE_1);
    else
        display_cmd2pic_noansw(LS35_SOUNDE_0);
}
...
void
lisy35_display_set_variant(unsigned char variant) {
    ...
    switch (variant) {
        case 0:
            display_cmd2pic_noansw(LS35DPCMD_VARIANT_0);
            break;
        case 1:
            display_cmd2pic_noansw(LS35DPCMD_VARIANT_1);
            break;
        case 2:
            display_cmd2pic_noansw(LS35DPCMD_VARIANT_2);
            break;
        case 3:
            display_cmd2pic_noansw(LS35DPCMD_VARIANT_3);
            break;
        case 4:
            display_cmd2pic_noansw(LS35DPCMD_VARIANT_4);
            break;
        default:
            display_cmd2pic_noansw(variant);
            break; //default set variant direct
...
*/
void
display_ss_LCD_string2row(unsigned char row, char* buf) {
    /* build control byte */
    mydata_display.bitv.IS_CMD = 1; //we are sending a command here
