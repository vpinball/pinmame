...
lisy_eeprom_256byte_write_24c04(char* wbuf, int block) {
    ...
    if (lisy_hardware_revision == 311) {
        adr1 = LISY80_HW311_EEPROM_ADDR1;
        adr2 = LISY80_HW311_EEPROM_ADDR2;
    } else if (lisy_hardware_revision == 100) {
        adr1 = LISY1_EEPROM_ADDR1;
        adr2 = LISY1_EEPROM_ADDR2;
    } else {
        adr1 = LISY80_HW320_EEPROM_ADDR1;
        adr2 = LISY80_HW320_EEPROM_ADDR2;
    }
...
lisy_eeprom_256byte_write_pic(char* wbuf, int block) {
    ...
    if (block == 0)
        old_eeprom_content = lisy_eeprom_buf_block0;
    else
        old_eeprom_content = lisy_eeprom_buf_block1;
    ...
    for (i = 0; i <= 255; i++) {
        ...
        if (*wbuf != *old_eeprom_content) {
            ...
            if (ls80dbg.bitv.basic == 1) {
                sprintf(debugbuf, "eeprom: write to PIC: Position:%d changed from %d to %d", i, *old_eeprom_content,
                        *wbuf);
...
lisy_eeprom_256byte_read_pic(char* rbuf, int block) {
    ...
    if (block == 0) {
        for (i = 0; i <= 255; i++) {
            *rbuf = lisy_eeprom_buf_block0[i];
            rbuf++;
        };
    } else {
        for (i = 0; i <= 255; i++) {
            *rbuf = lisy_eeprom_buf_block1[i];
            rbuf++;
        };
    }
...
//-1 in case of errror
int
lisy_eeprom_checksignature(void) {
...
int
lisy_eeprom_1byte_read(unsigned char address, int block) {
...
//block 0 means displaypic, block one coilpic
int
lisy_eeprom_1byte_write(unsigned char address, unsigned char data, int block) {
