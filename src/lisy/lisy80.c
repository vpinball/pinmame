...
lisy80_init(void) {
    ...
    lisy80_SOL9 = 0;
    old_neg_sounds =
        0; //RTH: change it in SOL pic as it do init with 0, where at startup output is '1111' because of inverter
    for (i = 0; i <= 47; i++)
        lisy80_lamp[i] = 0;
    for (i = 0; i <= 11; i++)
        lisy80_flip_flop[i] = 0;
    ...
    if (lisy80_has_own_sounds) {
        ...
        if (lisy80_file_get_soundopts() < 0) {
            ...
            if (ls80dbg.bitv.sound) {
                int i;
                for (i = 1; i <= 63; i++)
                    fprintf(stderr, "Sound[%d]: %d %d %d \n", i, lisy80_sound_stru[i].volume, lisy80_sound_stru[i].loop,
...
*/
int
lisy80_switch_handler(int riot0b) {
    ...
    if (CHECK_BIT(swMatrix[8], 4)) //is bit set?
    {
        if (nvram_delayed_write == 0) //only if it is not set anyway
        {
            ...
            if (ls80dbg.bitv.basic)
                lisy80_debug("NVRAM delayed write initiated by CREDIT button");
        }
    }
...
//BCD Version
void
lisy80_display_handler_bcd(int riot1a, int riot1b) {
    ...
    if ((riot1a & ~old_riot1a & 0x10) && (riot1a & ~old_riot1a & 0x20) && (riot1a & ~old_riot1a & 0x40)
        && (segment_data == 15)) {
...
*/
    ...
    if (old_digit == digit) {
        if (riot1a & ~old_riot1a & 0x10)
            seg_A = segment_data; //push_A ?
        if (riot1a & ~old_riot1a & 0x20)
            seg_B = segment_data; //push_B ?
        if (riot1a & ~old_riot1a & 0x40)
            seg_C = segment_data; //push_C ?
    ...
    } else //new sequence, we write now the data of the old_digit
    {
        ...
        if (old_digit <= 5) {
            if (player1[old_digit] != seg_A) {
                player1[old_digit] = seg_A;
                display_show_int(1, old_digit, seg_A);
            }
            if (player3[old_digit] != seg_B) {
                player3[old_digit] = seg_B;
                display_show_int(3, old_digit, seg_B);
            }
            if (player5[old_digit] != seg_C) {
                player5[old_digit] = seg_C;
                display_show_int(5, old_digit, seg_C);
            }
        }
        //digit 6..10 serves Player2 ; 4 and 6(Timer)
        if ((old_digit > 5) && (old_digit <= 11)) {
            if (player2[old_digit - 6] != seg_A) {
                player2[old_digit - 6] = seg_A;
                display_show_int(2, old_digit - 6, seg_A);
            }
            if (player4[old_digit - 6] != seg_B) {
                player4[old_digit - 6] = seg_B;
                display_show_int(4, old_digit - 6, seg_B);
            }
            if (player6[old_digit - 6] != seg_C) {
                player6[old_digit - 6] = seg_C;
                display_show_int(6, old_digit - 6, seg_C);
            }
        }
        if (old_digit > 11) {
            if (old_digit == 12) //7.Digit SYS80A Player 2 and 4
            {
                if (player2[6] != seg_A) {
                    player2[6] = seg_A;
                    display_show_int(2, 6, seg_A);
                }
                if (player4[6] != seg_B) {
                    player4[6] = seg_B;
                    display_show_int(4, 6, seg_B);
                }
            }
            if (old_digit == 15) //7.Digit SYS80A Player 1 and 3
            {
                if (player1[6] != seg_A) {
                    player1[6] = seg_A;
                    display_show_int(1, 6, seg_A);
                }
                if (player3[6] != seg_B) {
                    player3[6] = seg_B;
                    display_show_int(3, 6, seg_B);
                }
            }
            if (status[old_digit - 12] != seg_C) {
                status[old_digit - 12] = seg_C;
                display_show_int(0, old_digit - 12, seg_C);
            }
        }
    } //new sequence
...
void
lisy80_display_handler_alpha(int a_or_b, int riot_port) {
...
void
lisy80_coil_handler_a(int data) {
    ...
    if (lisy80_riot2_porta.bitv1.I1G == 0) //decoder one enabled
    {
        ...
        switch (lisy80_riot2_porta.bitv1.AB1) {
            case 3:
                lisy80_Z28.bitv.SOL1 = 1;
                break;
            case 2:
                lisy80_Z28.bitv.SOL2 = 1;
                break;
            case 1:
                lisy80_Z28.bitv.SOL3 = 1;
                break;
            case 0:
                lisy80_Z28.bitv.SOL4 = 1;
                break;
        }
    } else
        lisy80_Z28.bitv2.DECODER1 = 0;
    ...
    if (lisy80_riot2_porta.bitv1.I2G == 0) //decoder two enabled
    {
        ...
        switch (lisy80_riot2_porta.bitv1.AB2) {
            case 3:
                lisy80_Z28.bitv.SOL5 = 1;
                break;
            case 2:
                lisy80_Z28.bitv.SOL6 = 1;
                break;
            case 1:
                lisy80_Z28.bitv.SOL7 = 1;
                break;
            case 0:
                lisy80_Z28.bitv.SOL8 = 1;
                break;
        }
    } else
        lisy80_Z28.bitv2.DECODER2 = 0;
    ...
    if (lisy80_riot2_porta.bitv2.SOUND_EN == 0) // Z31 (7408 'AND') sound control enabled
    {
        if (old_neg_sounds != lisy80_riot2_porta.bitv2.SOUND) {
            ...
            if (lisy80_has_own_sounds)
                lisy80_play_wav(new_sound + sound16);
    ...
    } else //not enabled, so value is '1111' and output is zero
    {
        ...
        if (old_neg_sounds != 15) {
            old_neg_sounds = 15;
            if (ls80dbg.bitv.sound)
                lisy80_debug("sound ctrl not enabled, setting sound to:0");
...
void
lisy80_coil_handler_b(int data) {
    ...
    if ((flip_flop_no < 1) || (flip_flop_no > 12))
        return;
    ...
    for (i = 0; i <= 3; i++) {
        if (lisy80_lamp[i + offset] != new_lamp[i]) {
            if (ls80dbg.bitv.lamps) {
                if (new_lamp[i])
                    sprintf(debugbuf, "LISY80_COIL_HANDLER: Transistor_Q:%d ON", i + 1 + offset);
                else
                    sprintf(debugbuf, "LISY80_COIL_HANDLER: Transistor_Q:%d OFF", i + 1 + offset);
                lisy80_debug(debugbuf);
            }
            ...
            if ((i + offset) == 0) {
                nvram_delayed_write = NVRAM_DELAY;
                if (ls80dbg.bitv.basic)
                    lisy80_debug("NVRAM delayed write initiated by GAME OVER Relay");
            }
            //remember
            lisy80_lamp[i + offset] = new_lamp[i];
            //and set the lamp/coil
            if (new_lamp[i])
                lisy80_coil_set(i + 1 + offset, 1);
            else
                lisy80_coil_set(i + 1 + offset, 0);
...
int
lisy80_get_gamename(char* gamename) {
...
//switch_nr is 0..3
int
lisy80_get_mpudips(int switch_nr) {
...
lisy80_nvram_handler_old(int read_or_write, uint8_t* GTS80_pRAM_GTB) {
    ...
    } else //we want to read
    {
        ...
        if (ret == 0)
            memcpy((char*)GTS80_pRAM, nvram_block.byte, 256);
        if (ls80dbg.bitv.basic) {
