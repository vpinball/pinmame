...
int
lisy_api_write(unsigned char* data, int count, int debug) {
...
int
lisy_api_read_string(unsigned char cmd, char* content) {
...
//return 0 otherwise
unsigned char
lisy_api_read_byte(unsigned char cmd, unsigned char* data) {
...
//return 0 otherwise
unsigned char
lisy_api_read_byte_wblock(unsigned char cmd, unsigned char* data) {
    uint8_t tries = 0;
    int ret;
...
//return 0 otherwise
unsigned char
lisy_api_read_2bytes(unsigned char cmd, unsigned char option, unsigned char* data1, unsigned char* data2) {

    unsigned char cmd_data[2];
    //send command
    cmd_data[0] = cmd;
    cmd_data[1] = option;
    if (lisy_api_write(cmd_data, 2, ls80dbg.bitv.basic) != 2)
        return (-2);

    //receive answer
    if (read(fd_api, data1, 1) != 1)
        return (-1);
    if (read(fd_api, data2, 1) != 1)
        return (-1);
...
lisy_api_print_hw_info(void) {
    ...
    for (no = 0; no < no_of_displays; no++) {
        //query each display
        if (lisy_api_read_2bytes(LISY_G_DISP_DETAIL, no, &data1, &data2) < 0)
            return (-1);
        // verbose type
        switch (data1) {
            case 0:
                strcpy(answer, "Display index is invalid or does not exist in machine.");
                break;
            case 1:
                strcpy(answer, "BCD7, BCD Code for 7 Segment Displays without comma");
                break;
            case 2:
                strcpy(answer, "BCD8, BCD Code for 8 Segment Displays (same as BCD7 but with comma");
                break;
            case 3:
                strcpy(answer, "SEG7, Fully addressable 7 Segment Display (with comma)");
                break;
            case 4:
                strcpy(answer, "SEG14,Fully addressable 14 Segment Display (with comma)");
                break;
            case 5:
                strcpy(answer, "ASCII, ASCII Code");
                break;
            case 6:
                strcpy(answer, "ASCII_DOT, ASCII Code with comma");
                break;
        }
    ...
    if (lisy_api_read_byte(LISY_G_NO_SW, &data) < 0)
        return (-1);
    ...
    for (i = 1; i <= 64; i++)
        my_switch_status[i] = lisy_api_get_switch_status(i);
...
int
lisy_api_send_SEG7_to_disp(unsigned char disp, int num, uint16_t* data) {
    ...
    //send it all
    if (lisy_api_write(seg7_data, len + 2, ls80dbg.bitv.displays) != len + 2) {
        fprintf(stderr, "ERROR write display\n");
        return (-2);
    }
...
int
lisy_api_send_SEG14_to_disp(unsigned char disp, int num, uint16_t* data) {
    ...
    //send it all
    if (lisy_api_write(seg14_data, len + 2, ls80dbg.bitv.displays) != len + 2) {
        fprintf(stderr, "ERROR write display\n");
        return (-2);
    }
...
int
lisy_api_send_str_to_disp(unsigned char num, char* str) {
    ...
    if (cmd != 255) {
        ...
        if (lisy_api_write(cmd_data, len + 2, ls80dbg.bitv.displays) != len + 2) {
            fprintf(stderr, "ERROR write display\n");
            return (-2);
        }
    }
...
unsigned char
lisy_api_ask_for_changed_switch(void) {
    ...
    if (write(fd_api, &cmd, 1) != 1)
        return (-2);
    //receive answer
    if (read(fd_api, &my_switch, 1) != 1)
        return (-1);
...
unsigned char
lisy_api_get_switch_status(unsigned char number) {
...
//lamp control
void
lisy_api_lamp_ctrl(int lamp_no, unsigned char action) {
    uint8_t cmd;
    unsigned char cmd_data[2];
...
//solenoid ON and OFF
void
lisy_api_sol_ctrl(int sol_no, unsigned char action) {
    uint8_t cmd;
    unsigned char cmd_data[2];
...
void
lisy_api_sol_set_hwrule(int sol_no, int special_switch) {
    uint8_t cmd;
...
void
lisy_api_display_set_prot(uint8_t display_no, uint8_t protocol) {
    uint8_t cmd;
...
void
lisy_api_sound_play_index(unsigned char board, unsigned char index) {
    uint8_t cmd;
    uint8_t data;
    int ret;
    unsigned char cmd_data[3];
...
void
lisy_api_sound_play_file(unsigned char board, char* filename) {
    uint8_t cmd, data;
    int i, len, ret;
...
void
lisy_api_show_boot_message(char* software_version, char* system_id, int game_no, char* gamename) {
    int i;
    char buf[22];
...
int
lisy_api_get_con_hw(char* idstr) {
    if (ls80dbg.bitv.basic) {
...
int
lisy_api_check_con_hw(char* idstr) {
    char nextbyte;
    int i, n, ret;
    unsigned char cmd;
    char content[10];
