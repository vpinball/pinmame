...
//send back string
void
send_back_string(int sockfd, unsigned char code, char* str) {
...
//send back byte
void
send_back_byte(int sockfd, unsigned char code, unsigned char answer) {
...
unsigned char
read_next_byte(int sockfd, unsigned char code) {
...
unsigned char
read_next_string(int sockfd, char* content) {
...
unsigned char
read_next_n_bytes(int sockfd, int n, char* content) {
...
void
set_display(int number, int code, int no_of_bytes, char* value) {
    char display_str[25];
        ...
        case LISY_HW_LISY80:
            if (lisy80_game.is80B) {
                ...
                if (number == 0) {
                    ...
                    if ((lisy80_game.type_from_csv[5] == 'A') && (number < 5))
                        sprintf(display_str, "%7s", value); //80A has 7digit for 1-4
                    else
                        sprintf(display_str, "%6s", value); //RTH: with v21 wehad a 7s here ??
                    display_show_str(number, display_str);
                }
            } //80 and 80A
            break;
...
void
play_sound(unsigned char code, unsigned char sound_no) {
...
void
set_coil_pulsetime(int sockfd, int number) {
    ...
    if (lisy_hardware_revision == LISY_HW_LISY35) {
        ...
        if (ls80dbg.bitv.basic) {
            sprintf(debugbuf, "HW rule switch %d, autofire  solenoid:%d pulse %d\n", sw,
                    hw_rule_for_switch[sw].solenoid, hw_rule_for_switch[sw].pulse_time);
            lisy80_debug(debugbuf);
        }
    }
}
...
check_switch(void) {
    ...
    switch (lisy_hardware_revision) {
        case LISY_HW_LISY1:
            ...
            if ((my_switch.bitv.switchno == 76) && (ls80opt.bitv.slam == 1))
    ...
    if (my_switch.bitv.switchno < 80) {
        if (action)
            lisy_switches[my_switch.bitv.switchno] = my_switch.bitv.status = 0;
        else
            lisy_switches[my_switch.bitv.switchno] = my_switch.bitv.status = 1;
        ...
        //autofire section
        if (hw_rule_for_switch[my_switch.bitv.switchno].is_set & (action == 0))
            autofire(my_switch.bitv.switchno);
...
int
mpf_set_interface_attribs(int fd, int speed) {
    struct termios tty;
...
*/
void
mpf_display_show_str(int display, char* data) {
    switch (lisy_hardware_revision) {
        case LISY_HW_LISY1:
...
unsigned char
send_disp_detail(int sockfd, int code, unsigned char display) {
    ...
    if (ls80dbg.bitv.basic) {
        sprintf(debugbuf, "queried display number:%d; send back type:%d,with  %d segments", display, disp_type,
                disp_num_segments);
        lisy80_debug(debugbuf);
    }
...
unsigned char
set_hw_rule(int sockfd, unsigned char sol) {
    unsigned char sw1;
    unsigned char sw2;
    unsigned char sw3;
    unsigned char pulse_time;
...
main(int argc, char* argv[]) {
    ...
    for (i = 0; i <= 159; i++)
        lisy_coil_pulse_time[i] = 150; //150msec pulsetime by default
    for (i = 0; i <= lisy_hw.no_sol; i++)
        hw_rule_for_switch[i].is_set = 0;
    //init internale vars switches
    for (i = 0; i < LISY_CONTROL_MAX_SWITCHES; i++)
        lisy_switches[i] = 0;
    ...
    if (mpf_sound_stream_init(ls80dbg.bitv.sound) == 0) {
        ...
        lisy_adjust_volume();
        sprintf(debugbuf, "/bin/echo \"Welcome to MPF Server Version %s\" | /usr/bin/festival --tts",
                s_mpf_software_version);
        system(debugbuf);
    }
        ...
        if (d) {
            while ((dir = readdir(d)) != NULL) {
                if (dir->d_type == DT_REG) //we only want regular files
                {
                    ...
                    if ((len >= 5)
                        && (strcmp(&(dir->d_name[len - 4]), ".mp3") == 0)) //we found something with extension mp3
                    {
    ...
    while (1) { //outer loop
            ...
            switch (code) {
                ...
                case LISY_FADE_MOD_LIGHTS: //fade number of modern lights - return none fake, we do not support that
                    ...
                    for (i = 0; i < parameter; i++)
                        dum = read_next_byte(newsockfd, code); //read all values
                    break;
        ...
        if (socket_mode)
            close(newsockfd); //in socket more we will get a new newsocket
    }                         //while (1);
