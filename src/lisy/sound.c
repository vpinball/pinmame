    ...
    for (i = 1; i <= 63; i++) {
        ...
        if (lisy80_sound_stru[i].volume != 0) {
            ...
            if (lisysound[i] == NULL) {
                ...
                sprintf(debugbuf, "preload file:%s as sound number %d (Volume:%d)", wav_file_name, i,
                        lisy80_sound_stru[i].volume);
        ...
        if (lisy35_sound_stru[i].soundnumber != 0) {
            ...
            if (lisysound[i] == NULL) {
                ...
                for (int alt = 2; alt <= LISY35_SOUND_MAX_ALTERNATIVE_FILES + 1; alt++) {
                    sprintf(wav_file_name, "/boot/%s/%s-%d.wav", lisy35_sound_stru[i].path, lisy35_sound_stru[i].name,
                            alt);
 ...
 */
void
lisy80_sound_stream_destroy(void) {
...
lisy80_play_wav(int sound_no) {
    ...
    if (ls80dbg.bitv.sound) {
        sprintf(debugbuf, "lisy80_play_wav: want to play sound number: %d (loop:%d not_int_loops:%d)", sound_no,
                lisy80_sound_stru[sound_no].loop, lisy80_sound_stru[sound_no].not_int_loops);
        lisy80_debug(debugbuf);
    }
    ...
    if (lisy80_sound_stru[sound_no].not_int_loops == 0) //check if this sound can stop loop sounds
    {
        ...
        for (i = 1; i <= 31; i++) {
            if (lisy80_sound_stru[i].loop & (Mix_Playing(i) != 0)) {
                if (ls80dbg.bitv.sound)
                    lisy80_debug("found running loop sound, we cancel it\n");
                Mix_HaltChannel(i);
            }
        } //all loop files
    }
    ...
    if (lisy80_sound_stru[sound_no].loop > 0)
        loopcount = -1; //infinitiv loops
 ...
 */
void
lisy35_play_wav(int sound_no) {

    int ret;

    if (ls80dbg.bitv.sound) {
        sprintf(debugbuf,
                "lisy35_play_wav: want to play sound number: %d mapped to sound:%d, for which %d versions are loaded",
                sound_no, lisy35_sound_stru[sound_no].soundnumber, lisy35_sound_stru[sound_no].how_many_versions);
        lisy80_debug(debugbuf);
    }
    ...
    if (lisy35_sound_stru[sound_no].soundnumber != 0) {
        ...
        if (lisy35_sound_stru[sound_no].option == LISY35_SOUND_OPTION_STOP_LOOP) {
            //yes, check running state for all loop sounds and cancel them if running
            for (int i = 1; i <= 257; i++) {
                if (Mix_Playing(i) != 0 && lisy35_sound_stru[i].soundnumber != 0
                    && lisy35_sound_stru[i].option == LISY35_SOUND_OPTION_LOOP) {
                    if (ls80dbg.bitv.sound)
                        lisy80_debug("found running loop, we cancel it\n");
                    Mix_HaltChannel(i);
                }
            }
        }
        ...
        if (lisy35_sound_stru[sound_no].option == LISY35_SOUND_OPTION_SPEECH) {
            for (int i = 1; i <= 257; i++) {
                if (Mix_Playing(i) != 0 && lisy35_sound_stru[i].soundnumber != 0
                    && lisy35_sound_stru[i].option == LISY35_SOUND_OPTION_SPEECH) {
                    Mix_HaltChannel(i);
                }
            }
        }
...
lisy_get_position(void) {
    ...
    //read 100 times
    for (i = 1; i <= 10; i++)
        poti_val = poti_val + lisy80_get_poti_val();
    ...
    for (i = 1; i <= 255; i++) {
        if ((lisy35_sound_stru[i].soundnumber != 0) & (lisy35_sound_stru[i].preload == 1)) {
            ...
            if (lisysound[i] == NULL) {
                ...
                // see if there are alternative sounds (suffix "-2", "-3", "-4"... at the end of file name, example "sound-3.wav")
                for (int alt = 2; alt <= LISY35_SOUND_MAX_ALTERNATIVE_FILES + 1; alt++) {
                    sprintf(sound_file_name, "/boot/%s/%s-%d", lisy35_sound_stru[i].path, lisy35_sound_stru[i].name,
                            alt);
