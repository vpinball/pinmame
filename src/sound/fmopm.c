...
void FMOPM_Clock(fmopm_t* chip, int clk)
{
    ...
    if (clk1)
    {
        ...
        chip->freq_kc_lfo_add_hnz[0] = (lfo_add & 192) != 0;

        int sum2 = chip->freq_kc_lfo[1];
        if (!chip->freq_lfo_sign[2] && (chip->freq_kc_lfo_l_of[1] || (chip->freq_kc_lfo[1] & 0xc0) == 0xc0))
            sum2 += 64;
        int corr_sub = chip->freq_lfo_sign[2] && !chip->freq_kc_lfo_l_of[1] && chip->freq_kc_lfo_add_hnz[1];
        if (corr_sub)
            sum2 += 0x1fc0;
    ...
    if (clk2)
    {
        ...
        int hi2 = (hi >> 2) & 1;
        int add = (pms == 7 && (hi & 6) == 6) || (pms >= 6 && (hi & 3) == 3);
        ...
        chip->freq_kc_lfo_add_hnz[1] = chip->freq_kc_lfo_add_hnz[0];
