...
/* Function prototypes */
static void qsound_update( int num, INT16 **buffer, int length );
static void qsound_write_data(uint8_t address, uint16_t data);
...
static void qsound_delay_update(qsound_delay *qd);
static int32_t qsound_fir_apply(qsound_fir *qf, int16_t input);
static int32_t qsound_delay_apply(qsound_delay *qd, const int32_t input);
