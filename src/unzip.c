...
void errormsg(const char* extmsg, const char* usermsg, const char* zipname) {
	char temp[250];
	sprintf(temp,"Error in zipfile %s\n%s\n%s\n", zipname, usermsg,"(This message may appear several more times, please be patient)");
	MessageBox(NULL,temp,"ROM ERROR",MB_OK | MB_ICONWARNING);
}
