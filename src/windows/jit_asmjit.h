...
/* Invalidate the block starting at 'addr' (self-modifying-code write). Safe to
 * call from the store thunks while that very block is executing (a block whose
 * STR hits its own start address retires itself): freeing of the code memory
 * is deferred to the next arm7_aj_get. */
