...
/*Main MPU Board uses 2 PIA Chips (0 & 1) - so we must start with 2.
  ------------------------------------------------------------------
  Generation 1: Squalk & Talk:		Uses 1 PIA - It will be #2
  Generation 2: Turbo Cheap Squalk: Uses 1 PIA - It will be #3
  Generation 3: Sounds Deluxe:		Uses 1 PIA - It will be #4
  Generation 4: Williams Sys 11C:	Uses 1 PIA - It will be #5

  Both Turbo Cheap Squalk & Sounds Deluxe use an AD7533 10-Bit Multiplying DAC
  (this needs to be emulated for better sound on games using that hardware)
*/
...
static void dac_a_w(int data) { 
	locals.dac_a = data; 
	//For now, we simply ignore top 2 bits, since 10-bit dac is not emulated.
	DAC_0_data_w(0,locals.dac_a);
}

static void dac_b_w(int data)
{
	locals.dac_b = data;
	//For now, we simply ignore top 2 bits, since 10-bit dac is not emulated.
	//DAC_0_data_w(0,locals.dac_a);
...
/* Generation 2 */
static READ_HANDLER(pia_s2_b_r) {logerror("reading sound data = %x\n",locals.snddata); return locals.snddata;}
static WRITE_HANDLER(pia_s2_a_w) { logerror("writing to DAC PA0-7: %x\n",data); dac_a_w(data); }
static WRITE_HANDLER(pia_s2_b_w) { logerror("writing to DAC PB6-7: %x\n",data); dac_b_w(data); }
static WRITE_HANDLER(pia_s2_cb2_w) { BY6803_UpdateSoundLED(data); }
static void pia_s2_irq (int state) {
  logerror("Triggering Sound IRQ\n"); 
  //cpu_set_irq_line(BY6803_SCPU1NO, M6809_IRQ_LINE, state ? ASSERT_LINE : CLEAR_LINE);
  cpu_set_irq_line(BY6803_SCPU1NO, M6809_IRQ_LINE, PULSE_LINE);
}

/* Generation 3 */
static READ_HANDLER(pia_s3_b_r) {logerror("reading sound data = %x\n",locals.snddata); return locals.snddata;}
static WRITE_HANDLER(pia_s3_a_w) { DAC_0_data_w(0,data); }
static WRITE_HANDLER(pia_s3_b_w) {}
...
static void pia_s3_irq (int state) {
	...
	cpu_set_irq_line(BY6803_SCPU1NO, MC68000_IRQ_4, PULSE_LINE);
}
...
MEMORY_READ_START(s4_readmem)
  ...
  { 0x4000, 0x4003, pia_5_r },                /* 4000-4fff */
...
MEMORY_WRITE_START(s4_writemem)
  ...
  { 0x4000, 0x4003, pia_5_w },                      /* 4000-4fff */
...
void by6803_sndinit1(){
	...
	//Configure PIA to use #2
}
void by6803_sndinit2(){ 
	memset(&locals, 0, sizeof(locals));
	//Configure PIA to use #3
	pia_config(3, PIA_ALTERNATE_ORDERING, &pia_s2);	//Alt Ordering for 16bit Write Access to Ports A+B
}
void by6803_sndinit3(){ 
	memset(&locals, 0, sizeof(locals));
	//Configure PIA to use #4
	pia_config(4, PIA_ALTERNATE_ORDERING, &pia_s3);	//Alt Ordering for 16bit Write Access to Ports A+B
}
void by6803_sndinit4(){ 
	memset(&locals, 0, sizeof(locals));
	//Configure PIA to use #5
    pia_config(5, PIA_STANDARD_ORDERING, &pia_s4);
    cpu_setbank(1, memory_region(BY6803_MEMREG_SROM));
}
...
WRITE_HANDLER(by6803_sndcmd4) { 
	logerror("Sound Command %x\n",data);
	locals.snddata = data;
	pia_set_input_cb1(5, 0);
	pia_set_input_cb1(5, 1);
	snd_cmd_log(data);
}
