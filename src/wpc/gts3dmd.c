...
#define GTS3DMD_FRAMES 3

static UINT8 DMDFrames[GTS3DMD_FRAMES][0x200];

int dmd_start(void)
{	
	int x,y;
	for(x = 0; x < GTS3DMD_FRAMES; x++)
		for(y = 0; y<0x200; y++)
			DMDFrames[x][y] = 0;
	return 0;
}
...
void gts3_dmd128x32_refresh(struct mame_bitmap *bitmap, int fullRefresh) {
  ...
  //coreGlobals_dmd.DMDFrames[coreGlobals_dmd.nextDMDFrame] = 
  //	memory_region(GTS3_MEMREG_DCPU1)+0x1000+offset;
  ...
  for (kk = 0, ii = 1; ii < (11*GTS3DMD_FRAMES); ii++) {
  ...
  for (kk = 0, ii = 1; ii < (11*GTS3DMD_FRAMES); ii++) {
    ...
    for (jj = 0; jj < 16; jj++) {
      ...
      /* been on in the last 3 frames*/
		...
		intens1=intens2=0;
		for(ll = 0; ll < GTS3DMD_FRAMES; ll++){
			intens1+=coreGlobals_dmd.DMDFrames[ll][kk] & 0x55;
			intens2+=coreGlobals_dmd.DMDFrames[ll][kk] & 0xaa;
		}
      *line++ = (intens2>>7) & GTS3DMD_FRAMES;
      *line++ = (intens1>>6) & GTS3DMD_FRAMES;
      *line++ = (intens2>>5) & GTS3DMD_FRAMES;
      *line++ = (intens1>>4) & GTS3DMD_FRAMES;
      *line++ = (intens2>>3) & GTS3DMD_FRAMES;
      *line++ = (intens1>>2) & GTS3DMD_FRAMES;
      *line++ = (intens2>>1) & GTS3DMD_FRAMES;
	  *line++ = (intens1)    & GTS3DMD_FRAMES;
      kk +=1;
    }
    *line = 0; /* to simplify antialiasing */
  }
