...
/* How the second line of each pair is filled.
/
/  0 - line doubling: each of the machine's 240 lines written out twice, byte for byte. What the
/      hardware does, so it is the default and every pixel is one the machine produced.
/  1 - interpolation: the odd output lines become the average of the machine's line above and the
/      one below. The even lines stay exact copies, so no original scanline is blurred and nothing
/      shifts by half a line; only the invented line between two of them is softened. The bottom
/      line of the frame has nothing below it and is doubled as before.
/
/  Cosmetic, and not free - two source lines read and 640 pixels averaged per output line - so it
/  stays off unless asked for, by -DP2K_LINE_INTERPOLATE=1 or by changing it here */
#ifndef P2K_LINE_INTERPOLATE
#define P2K_LINE_INTERPOLATE 0
#endif

#if P2K_LINE_INTERPOLATE && (P2K_LINE_DOUBLE != 2)
#error "P2K_LINE_INTERPOLATE describes one new line between two real ones and needs P2K_LINE_DOUBLE == 2"
#endif
