...
WRITE_HANDLER( s80bs_ay8910_latch_w ){ s80bslocals.ay_latch = data;}

//Init 
void S80Bs_sound_init(void) { 
...
WRITE_HANDLER( s80bs_nmi_rate_w ){ s80bslocals.nmi_rate = data; }
...
static WRITE_HANDLER( common_sound_control_w )
{
	...
	if (s80bslocals.nmi_timer)
	{
		...
		s80bslocals.nmi_timer = 0;
	}

	if (data & 0x01)
	{
		/* base clock is 250kHz divided by 256 */
		double interval = TIME_IN_HZ(250000.0/256/(256-s80bslocals.nmi_rate));
		s80bslocals.nmi_timer = timer_pulse(interval, 0, nmi_callback);
	}
...
WRITE_HANDLER( s80bs1_sound_control_w )
{
	...
	if ((last & 0x04) == 0x04 && (data & 0x04) == 0x00)
	{
		...
		if (data & 0x08)
		{
			...
			if (data & 0x10)
				AY8910_control_port_0_w(0,s80bslocals.ay_latch);
			else
				AY8910_write_port_0_w(0,s80bslocals.ay_latch);
		}
		else
		{
			/* bit 4 goes to the 8913 BC1 pin */
			if (data & 0x10)
				AY8910_control_port_1_w(0,s80bslocals.ay_latch);
			else
				AY8910_write_port_1_w(0,s80bslocals.ay_latch);
		}
	}
...
{
	s80bslocals.dac_volume = data ^ 0xff;
	DAC_data_16_w(0, s80bslocals.dac_volume * s80bslocals.dac_data);
}
//DAC Handling.. Set data to send
WRITE_HANDLER( s80bs_dac_data_w )
{
	s80bslocals.dac_data = data;
	DAC_data_16_w(0, s80bslocals.dac_volume * s80bslocals.dac_data);
}

//Process command from Main CPU
void S80Bs_soundlatch(int data)
{
...
/***************************/
//Y-CPU
MEMORY_READ_START( S80Bs1_sreadmem )
	{ 0x0000, 0x03ff, MRA_RAM },
	{ 0x6000, 0x6000, s80bs1_sound_input_r },
	{ 0xa800, 0xa800, soundlatch_r },
	{ 0xc000, 0xffff, MRA_ROM },
MEMORY_END
MEMORY_WRITE_START( S80Bs1_swritemem )
...
MEMORY_END
//D-CPU
MEMORY_READ_START( S80Bs1_sreadmem2 )
	{ 0x0000, 0x03ff, MRA_RAM },
	{ 0x8000, 0x8000, soundlatch_r },
	{ 0xe000, 0xffff, MRA_ROM },
MEMORY_END
MEMORY_WRITE_START( S80Bs1_swritemem2 )
	{ 0x0000, 0x03ff, MWA_RAM },
...
/* D-CPU */
/*********/
MEMORY_READ_START(S80Bs2_sreadmem2)
{ 0x0000, 0x07ff, MRA_RAM },
{ 0x4000, 0x4000, soundlatch_r},
{ 0x8000, 0xffff, MRA_ROM },
MEMORY_END
MEMORY_WRITE_START(S80Bs2_swritemem2)
{ 0x0000, 0x07ff, MWA_RAM },
...
/* Y-CPU */
/*********/
MEMORY_READ_START(S80Bs3_sreadmem)
{ 0x0000, 0x07ff, MRA_RAM },
{ 0x6800, 0x6800, soundlatch_r},
{ 0x7000, 0x7000, s80bs_cause_dac_nmi_r},
{ 0x8000, 0xffff, MRA_ROM },
MEMORY_END
MEMORY_WRITE_START(S80Bs3_swritemem)
{ 0x0000, 0x07ff, MWA_RAM },
...
/* D-CPU */
/*********/
MEMORY_READ_START(S80Bs3_sreadmem2)
{ 0x0000, 0x07ff, MRA_RAM },
{ 0x4000, 0x4000, soundlatch_r},
{ 0x8000, 0xffff, MRA_ROM },
MEMORY_END
MEMORY_WRITE_START(S80Bs3_swritemem2)
{ 0x0000, 0x07ff, MWA_RAM },
{ 0x8000, 0x8000, s80bs_dac_vol_w },
{ 0x8001, 0x8001, s80bs_dac_data_w},
MEMORY_END
...
/ Sound interface
/-----------------*/
struct DACinterface S80Bs_dacInt =
{ 
  2,			/*2 Chips - but it seems we only access 1?*/
 {25,25}		/* Volume */
};

struct AY8910interface S80Bs_ay8910Int = {
	2,			/* 2 chips */
	2000000,	/* 2 MHz */
	{ 25, 25 }, /* Volume */
	{ 0 },
...
struct YM2151interface S80Bs_ym2151Int =
{
	1,			/* 1 chip */
	4000000,	/* 4 MHz */
