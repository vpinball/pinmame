...
/*-------------------------------------------------------------------
/ Sleic Pin-Ball (free play + end-of-game scores)
/
/  A MOD of the 1993 set with two independent patches applied to chip sp03.  Only
/  that chip changes; sp01, sp02 and sp04 are the parent's.
/
/  END-OF-GAME SCORES.  The machine has no end-of-game score screen: the sequence
/  that follows the bonus count-up goes straight on, so the final scores are never
/  shown.  The patch composes one -- every player's eight score digits, two per
/  row in the machine's own 8-row face at F000:85EB, with PRESS START beneath --
/  and holds it until START is pressed, then hands the sequence back to the step it
/  would have run.
/
/  The screen is reached through a spare slot in the sequence table at E000:4F7C.
/  Entry 25 (E000:4FAE) is unreachable in stock firmware: the advance tail wraps at
/  0x18 and no write to the index [0000:017D] anywhere in the image produces 25.
/  Both game-over tails -- E000:197C for nine games in ten and E000:1962 on the
/  tenth, which E000:1919 selects on an NVRAM game counter -- are redirected to it,
/  each recording the index its tail would have written.  Entry 23, the one
/  attract's own free-running cycle reaches, is left alone, so the screen cannot
/  appear in attract.  Both of the attract loop's credit tests (E000:00D8, the
/  loop's only entrance, and E000:00EC) are hooked so the screen appears whether or
/  not credits are standing, which is what makes it work alongside free play.
/
/  The cave snapshots the player count and all four score blocks in those
/  trampolines, because the attract loop's entry preamble resets the game state
/  before the screen's own code can run: [0x106] at E000:00B8, and call 0x7bd at
/  E000:00CB, which wipes 0x1C5-0x26E, every player's score block.
/
/  FREE PLAY.  Sleic Pin-Ball has no free-play adjustment of its own.  Credits are
/  a triplicated NVRAM byte at 0x140-0x142 behind a three-way equality check, and
/  [0000:0100] is the RAM cache every consumer reads.  The patch floors that cache
/  to at least 1 at both of its writers, E000:147E and E000:14C8, and returns the
/  floored value so the credit display agrees.  The NVRAM triple is never written,
/  so a free game consumes nothing.
/
/  The two patches touch no common byte and stack in either order to the same
/  image.  Generated by sleic-iomoon/scripts/sleic_pin_ball_free_play_patch.py and
/  sleic_pin_ball_press_start_patch.py.
/-------------------------------------------------------------------*/
INITGAME(sleicpnf, sleic_dispDMD, 1)
SLEIC_ROMSTART4(sleicpnf,"sp01-1_1.rom", CRC(240015bb) SHA1(0e647718173ad59dafbf3b5bc84bef3c33886e23),
						 "sp02-1_1.rom", CRC(0e4851a0) SHA1(0692ee2df0b560e2013db9c03fd27c6eb12e618d),
