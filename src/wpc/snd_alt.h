...
//DAR@20230516 This could be leveraged to fix the ducking issues
//             (https://github.com/vpinball/pinmame/issues/113)
...
// Support for lookup-based altsound
...
void load_samples(Pin_samples* pinsamples_out);
...
void parse_altsound_csv(size_t BASE_PATH_LEN, Pin_samples* psd);
void parse_altsound_folders(size_t BASE_PATH_LEN, Pin_samples* psd);

// Main entrypoint
void alt_sound_handle(int boardNo, int cmd)
{
	LOG(("IN ALT_SOUND_HANDLE\n")); //DAR_DEBUG

	// Seed random number generator.  Only want to do this once.
    ...
    //DAR@20230516 Some of these probably needs to be globals, so we can
	//             split this monolithic block of code into functions
	static unsigned int cmd_counter = 0;
	static int cmd_storage = -1;
	...
	LOG(("Attenuation: %f\n ", attenuation)); //DAR_DEBUG

	float master_vol = 1.0f;
	LOG(("Master Volume (Pre Attenuation): %f\n ", master_vol)); //DAR_DEBUG
	...
	if (cached_machine_name != 0 && strstr(g_szGameName, cached_machine_name) == 0) // another game has been loaded? -> previous data has to be free'd
	{
		...
		LOG(("New Game loaded?  Clearing cmds\n")); //DAR_DEBUG
	...
	if (cmd_storage == -1)
	{
		...
		if (psd.num_files > 0)
		{
			...
			//DAR@20230516 If we simply cleaned up and exited in these
			//             situations, we wouldn't need to keep bracketing
			//             nominal code in IF..THEN clauses
			cmd_storage = 0;
	}
	...
	if (psd.num_files > 0)
	{
		UINT64 mame_gen = core_gameData->gen; // DAR_DEBUG
		LOG(("MAME_GEN: %d\n", mame_gen)); //DAR_DEBUG

		int ch;
		// force internal PinMAME volume mixer to 0 to mute emulated sounds & musics
		// required for WPC89 sound board
		LOG(("PSD.NUM_FILES >0...MUTING SOUNDS\n")); //DAR_DEBUG
			...
			(core_gameData->gen == GEN_WPC95))
		{
				...
				((cmd_buffer[2] == 0x00) && (cmd_buffer[1] == 0x00) && (cmd_buffer[0] == 0x00))) // glitch in command buffer?
			{
				...
				LOG(("Volume set... clearing buffer\n")); //DAR_DEBUG
				for (int i = 0; i < ALT_MAX_CMDS; ++i)
					cmd_buffer[i] = ~0;
...
//
// This callback function ducks the gain on the sound playing on channel 0
//
void CALLBACK ducking_callback(HSYNC handle, DWORD channel, DWORD data, void *user)
{
...
// Load Samples
//
void load_samples(Pin_samples* psd) {
	LOG(("LOADING SAMPLES\n")); //DAR_DEBUG
	...
	parse_altsound_csv(BASE_PATH_LEN, psd);

	// If no CSV, try reading from older folder-based structure directories
	// OTHERWISE scan folders for old folder-based structure of the alternate sound package
	if (psd->num_files == 0)
		parse_altsound_folders(BASE_PATH_LEN, psd);

	if (psd->num_files == 0)
		LOG(("FAILED TO PARSE ALTSOUND SAMPLES\n"));
}
...
//		*lpSlash = '\0';
	...
	//DAR_TODO what is this for?  Should it be moved?
//	dir = opendir(cwd);
//	closedir(dir);
}
